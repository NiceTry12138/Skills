...
### Passing structured data as a CLI argument

Wrong (Bash habit, fragile):
```powershell
& $exe '--data' "{\"key\":\"$value\"}"
```

Acceptable (PS 7 only, simple cases):
```powershell
$payload = @{ key = $value } | ConvertTo-Json -Compress
& $exe '--data' $payload
```

Most reliable (cross-version, complex payloads):
```powershell
$tempFile = [System.IO.Path]::GetTempFileName()
try {
    $payload = @{ key = $value } | ConvertTo-Json -Depth 10
    [System.IO.File]::WriteAllText(
        $tempFile,
        $payload,
        [System.Text.UTF8Encoding]::new($false)
    )
    & $exe '--data-file' $tempFile
    $exitCode = $LASTEXITCODE
}
finally {
...
6. Invoke the native executable directly with `& $exe @cliArgs`.
